...
InputLayout::InputLayout(ID3D11Device* device, ID3D11DeviceContext* deviceContext, ID3DBlob* pVertexShaderBytecode) {
	...
	ThrowIfFailed(
		device->CreateInputLayout(ied.data(), (UINT)ied.size(), pVertexShaderBytecode->GetBufferPointer(), pVertexShaderBytecode->GetBufferSize(), &pInputLayout)
	);
}

InputLayout::InputLayout(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const std::vector<D3D11_INPUT_ELEMENT_DESC>& layout, ID3DBlob* pVertexShaderBytecode) {

	ThrowIfFailed(
		device->CreateInputLayout(layout.data(), (UINT)layout.size(), pVertexShaderBytecode->GetBufferPointer(), pVertexShaderBytecode->GetBufferSize(), &pInputLayout)
	);
}

void InputLayout::Bind(ID3D11Device* device, ID3D11DeviceContext* deviceContext) noexcept {
