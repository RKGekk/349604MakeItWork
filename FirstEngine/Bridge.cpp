...
Bridge::Bridge(ID3D11Device* device, ID3D11DeviceContext* deviceContext, CameraClass* cam, DirectX::XMFLOAT4X4* world) : Drawable(world), m_camera(cam) {

	IndexedTriangleList model = IndexedTriangleList::LoadUVNormals("Bridge1.obj");
	...
	std::unique_ptr<ShaderResource> srv0 = std::make_unique<ShaderResource>(device, deviceContext, TextureHolder::GetTexture("vented-metal-panel1_albedo.bmp").GetTexture());
	AddBind(std::move(srv0));
	std::unique_ptr<ShaderResource> srv1 = std::make_unique<ShaderResource>(device, deviceContext, TextureHolder::GetTexture("vented-metal-panel1_normal-ogl.bmp").GetTexture(), 1);
...
void Bridge::Update(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float dt, float tt) {
