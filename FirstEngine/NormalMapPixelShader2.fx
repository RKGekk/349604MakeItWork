...
float4 TexturePixelShader(PixelInputType input) : SV_TARGET{
	...
	//mt.Specular = shaderSpecularTexture.Sample(SampleType, input.tex) + 0.5f;
	//mt.Specular = float4(0.9f, 0.9f, 0.9f, 1.0f);
	//mt.Specular.w = 16.0f;
	...
	//ComputeSpotLight(mt, gSpotLight, input.PosW, bumpedNormalW, toEyeW, A, D, S);
	//ComputeSpotLight(gMaterial, gSpotLight, input.PosW, input.NormalW, toEyeW, A, D, S);
	ComputeSpotLight(gMaterial, gSpotLight, input.PosW, bumpedNormalW, toEyeW, A, D, S);
	ambient += A;
	diffuse += D;
	spec += S;
