...
Planet::Planet(ID3D11Device* device, ID3D11DeviceContext* deviceContext, CameraClass* cam, DirectX::XMFLOAT4X4* world) : Drawable(world), m_camera(cam) {

	float planetRadius = 1.0f;

	GeometryGenerator::MeshData sphere;
	GeometryGenerator geoGen;
	geoGen.CreateSphere(planetRadius, 30, 30, sphere);
	...
	auto pvs = std::make_unique<VertexShader>(device, deviceContext, L"NormalMapVertexShader.fx");
	ID3DBlob* pvsbc = pvs->GetBytecode();
	AddBind(std::move(pvs));

	auto pps = std::make_unique<PixelShader>(device, deviceContext, L"NormalMapPixelShader.fx");
	ID3DBlob* ppsbc = pps->GetBytecode();
	AddBind(std::move(pps));

	std::unique_ptr<ShaderResource> srv0 = std::make_unique<ShaderResource>(device, deviceContext, TextureHolder::GetTexture("2k_earth_daymap.bmp").GetTexture());
	AddBind(std::move(srv0));
	std::unique_ptr<ShaderResource> srv1 = std::make_unique<ShaderResource>(device, deviceContext, TextureHolder::GetTexture("2k_earth_specular_map.bmp").GetTexture(), 1);
	AddBind(std::move(srv1));
	std::unique_ptr<ShaderResource> srv2 = std::make_unique<ShaderResource>(device, deviceContext, TextureHolder::GetTexture("2k_earth_normal_map.bmp").GetTexture(), 2);
	AddBind(std::move(srv2));
...
void Planet::Update(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float dt, float tt) {
