...
CryoCapsule::CryoCapsule(ID3D11Device* device, ID3D11DeviceContext* deviceContext, CameraClass* cam, DirectX::XMFLOAT4X4* world) : Drawable(world), m_camera(cam) {

	IndexedTriangleList model = IndexedTriangleList::LoadUVNormals("cryo8_2.obj");
	...
	std::unique_ptr<ShaderResource> srv0 = std::make_unique<ShaderResource>(device, deviceContext, TextureHolder::GetTexture("greasy-metal-pan1-albedo.bmp").GetTexture());
	AddBind(std::move(srv0));
	std::unique_ptr<ShaderResource> srv1 = std::make_unique<ShaderResource>(device, deviceContext, TextureHolder::GetTexture("greasy-metal-pan1-normal.bmp").GetTexture(), 1);
...
void CryoCapsule::Update(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float dt, float tt) {
