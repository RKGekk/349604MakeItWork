...
Sky::Sky(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const std::wstring& texture, CameraClass* cam) : m_camera(cam) {
	...
	auto pvs = std::make_unique<VertexShader>(device, deviceContext, L"SkyVertexShader.fx");
	ID3DBlob* pvsbc = pvs->GetBytecode();
	AddBind(std::move(pvs));

	auto pps = std::make_unique<PixelShader>(device, deviceContext, L"SkyPixelShader.fx");
	ID3DBlob* ppsbc = pps->GetBytecode();
	AddBind(std::move(pps));

	DirectX::CreateDDSTextureFromFile(device, deviceContext, texture.c_str(), m_texture.GetAddressOf(), m_textureView.GetAddressOf());

	std::unique_ptr<ShaderResource> srv = std::make_unique<ShaderResource>(device, deviceContext, m_textureView.Get());
	...
	std::unique_ptr<DepthStencilState> depthStencilState = std::make_unique<DepthStencilState>(device, deviceContext, depthStencilDesc);
	...
	std::unique_ptr<RasterizerState> rasterizerState = std::make_unique<RasterizerState>(device, deviceContext, rasterDesc);
...
void Sky::Update(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float dt, float tt) {
