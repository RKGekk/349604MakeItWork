...
Ship::Ship(ID3D11Device* device, ID3D11DeviceContext* deviceContext, CameraClass* cam, DirectX::XMFLOAT4X4* world) : Drawable(world), m_camera(cam) {

	IndexedTriangleList model = IndexedTriangleList::LoadUVNormals("sheep.obj");
	...
	auto pvs = std::make_unique<VertexShader>(device, deviceContext, L"BasicPhongColorVertexShader.fx");
	ID3DBlob* pvsbc = pvs->GetBytecode();
	AddBind(std::move(pvs));

	auto pps = std::make_unique<PixelShader>(device, deviceContext, L"BasicPhongColorPixelShader.fx");
...
void Ship::Update(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float dt, float tt) {
